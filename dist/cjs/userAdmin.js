...
const g = a5base_1.setGlobal.default();
const pool = g.pool;
...
async function userChangePassword(req, res) {
    ...
    if (!req.body.newpassword) {
        ...
        query = pool.query("UPDATE users SET token=$1 WHERE name=$2 RETURNING name,pass_enc,token,role", [(0, crypto_1.createHash)('sha256').update(req.body.newtoken).digest('hex'), req.body.username]);
    }
    else if (req.body.newtoken && req.body.newtoken != "") {
        query = pool.query("UPDATE users SET pas_enc=$1, token=$2 WHERE name=$3 RETURNING name,pass_enc,token,role", [(0, crypto_1.createHash)('sha256').update(req.body.newpassword).digest('hex'), (0, crypto_1.createHash)('sha256').update(req.body.newtoken).digest('hex'), req.body.username]);
    }
    else {
        query = pool.query("UPDATE users set pass_enc=$1 where name=$2 RETURNING name,pass_enc,role", [(0, crypto_1.createHash)('sha256').update(req.body.newpassword).digest('hex'), req.body.username]);
    }
    try {
...
class User extends a5base_1.baseModel.baseModel {
    constructor(fields) {
        super(fields);
        this.id = fields.id;
    ...
    static fromCSVMany(csv_string) {
        const data = (0, csv_string_1.parse)(csv_string);
        return data.map(r => {
    ...
    static fromCSV(array_or_string) {
        ...
        if (!Array.isArray(array_or_string)) {
            const parsed = (0, csv_string_1.parse)(array_or_string);
    ...
    encryptToken() {
        return (this.token) ? (typeof this.token == "string") ? (0, crypto_1.createHash)('sha256').update(this.token).digest('hex') : Buffer.from(this.token) : undefined;
    }
    encryptPassword() {
        return (this.password) ? (0, crypto_1.createHash)('sha256').update(this.password).digest('hex') : (this.pass_enc) ? Buffer.from(this.pass_enc) : undefined;
    }
    async create() {
        if (!pool) {
            throw new Error("Pool not initialized");
